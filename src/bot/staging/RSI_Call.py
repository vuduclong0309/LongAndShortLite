...
print("%s-%s-SMART-USD-%s-CALL" % (symbol_glob, expdate, str(strike_glob)))
...
class StochRSI(bt.Strategy):
    ...
    def next(self):
        ...
        if self.getpositionbyname(self.strike).size <= 0:
            if self.rsi < 30:
                self.order = self.buy(data=self.strike, size=1, trailpercent = 5) # buy when closing price today crosses above MA.
        else:
            if self.rsi > 70 \
                or self.getpositionbyname(self.strike).price * 0.9 > self.datas[1].close[0] \
                or self.getpositionbyname(self.strike).price * 1.2 < self.datas[1].close[0]:
...
def run(args=None):
    ...
    data = store.getdata(dataname="%s-%s-SMART-USD-%s-CALL" % (symbol_glob, expdate, str(strike_glob)), **stockkwargs)
